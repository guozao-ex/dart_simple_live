...
abstract final class DmUtils {
  ...
  /// 弹幕字重索引（0-8，对应 [FontWeight.w100] - [FontWeight.w900]）转 [FontWeight]。
  ///
  /// 本地补丁：原实现直接使用 `FontWeight.values[fontWeight]`，外部传入越界索引会抛
  /// [RangeError]（上层 1-9 档设置传到 9 时必然崩溃），这里统一做钳位。
  static FontWeight fontWeightOf(int fontWeight) =>
      FontWeight.values[_clampWeightIndex(fontWeight)];

  static int _clampWeightIndex(int value) {
    if (value < 0) {
      return 0;
    }
  ...
  /// 字重索引对应的可变字体（variable font）`wght` 轴取值。
  ///
  /// 本地补丁：绘制只设置 fontSize/fontWeight 时，可变字体不会切换字重实例
  /// （SkParagraph 仅按 fontVariations 克隆字型），必须显式传入本列表字重才会生效。
  static List<ui.FontVariation> fontVariationsOf(int fontWeight) =>
      <ui.FontVariation>[
        ui.FontVariation('wght', fontWeightOf(fontWeight).value.toDouble()),
      ];
  ...
  static ui.Paragraph generateParagraph({
    ...
    final fontVariations = fontVariationsOf(fontWeight);
